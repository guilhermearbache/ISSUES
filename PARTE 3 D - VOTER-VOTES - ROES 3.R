...
lapcitizen <- completeFun(lapfinal, c("vb2", "ROES3"))

elavotes <- completeFun(elavotes, "ROES3")
...
bra_smp_cong <- bra_smp %>% dplyr::select (ROES3, pop) 
bra_lap_cong <- bra_lapvoter %>% dplyr::select (ROES3, pop)


bra_congvotes <-rbind(bra_smp_cong, bra_lap_cong)

bra_congvotes <- bra_congvotes %>% dplyr::rename (samps = ROES3) 
...
# CONGRUÊNCIA
BRA_emd_votes_ROES3 <- emd.dis(bra_congvotes)
...
bol_lap_cong <- bol_lapvoter %>% select (ROES3, pop)


bol_congvotes <-rbind(bol_smp_cong, bol_lap_cong)

bol_congvotes <- bol_congvotes %>% dplyr::rename (samps = ROES3) 
...
BOL_emd_votes_ROES3 <- emd.dis(bol_congvotes)
...
CHL_lap_cong <- CHL_lapvoter %>% select (ROES3, pop)


CHL_congvotes <-rbind(CHL_smp_cong, CHL_lap_cong)

CHL_congvotes <- CHL_congvotes %>% dplyr::rename (samps = ROES3) 
...
CHL_emd_votes_ROES3<- emd.dis(CHL_congvotes)
...
COL_lap_cong <- COL_lapvoter %>% select (ROES3, pop)


COL_congvotes <-rbind(COL_smp_cong, COL_lap_cong)

COL_congvotes <- COL_congvotes %>% dplyr::rename (samps = ROES3) 
...
COL_emd_votes_ROES3 <- emd.dis(COL_congvotes)
...
CRI_lap_cong <- CRI_lapvoter %>% select (ROES3, pop)


CRI_congvotes <-rbind(CRI_smp_cong, CRI_lap_cong)

CRI_congvotes <- CRI_congvotes %>% dplyr::rename (samps = ROES3) 
...
CRI_emd_votes_ROES3 <-emd.dis(CRI_congvotes)
...
DOM_lap_cong <- DOM_lapvoter %>% select (ROES3, pop)


DOM_congvotes <-rbind(DOM_smp_cong, DOM_lap_cong)

DOM_congvotes <- DOM_congvotes %>% dplyr::rename (samps = ROES3) 
...
DOM_emd_votes_ROES3 <-emd.dis(DOM_congvotes)
...
GTM_lap_cong <- GTM_lapvoter %>% select (ROES3, pop)


GTM_congvotes <-rbind(GTM_smp_cong, GTM_lap_cong)

GTM_congvotes <- GTM_congvotes %>% dplyr::rename (samps = ROES3) 
...
GTM_emd_votes_ROES3 <- emd.dis(GTM_congvotes)
...
HND_lap_cong <- HND_lapvoter %>% select (ROES3, pop)


HND_congvotes <-rbind(HND_smp_cong, HND_lap_cong)

HND_congvotes <- HND_congvotes %>% dplyr::rename (samps = ROES3) 
...
HND_emd_votes_ROES3 <- emd.dis(HND_congvotes)
...
NIC_lap_cong <- NIC_lapvoter %>% select (ROES3, pop)


NIC_congvotes <-rbind(NIC_smp_cong, NIC_lap_cong)

NIC_congvotes <- NIC_congvotes %>% dplyr::rename (samps = ROES3) 
...
NIC_emd_votes_ROES3 <- emd.dis(NIC_congvotes)
...
URY_lap_cong <- URY_lapvoter %>% select (ROES3, pop)


URY_congvotes <-rbind(URY_smp_cong, URY_lap_cong)

URY_congvotes <- URY_congvotes %>% dplyr::rename (samps = ROES3) 
...
URY_emd_votes_ROES3 <- emd.dis(URY_congvotes)
URY_dmeans  <- dmeans(URY_congvotes)



write.xlsx(list(BOL =BOL_emd_votes_ROES3, BRA =BRA_emd_votes_ROES3, CHL=CHL_emd_votes_ROES3, 
                COL=COL_emd_votes_ROES3, CRI=CRI_emd_votes_ROES3, DOM=DOM_emd_votes_ROES3, GTM=GTM_emd_votes_ROES3, HND=HND_emd_votes_ROES3, 
                NIC=NIC_emd_votes_ROES3, URY=URY_emd_votes_ROES3),
           "EMD - Voter_votes - ROES3.xlsx")
